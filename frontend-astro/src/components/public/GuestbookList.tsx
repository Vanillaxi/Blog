...
export function GuestbookList({ items, onReply }: { items: GuestbookMessage[]; onReply?: (item: GuestbookMessage) => void }) {
  ...
  return (
    <section className="overflow-hidden rounded-[1.75rem] border border-white/55 bg-white/55 shadow-[0_18px_50px_rgba(148,163,184,0.14)] backdrop-blur-xl">
      {visibleItems.map((item, index) => (
        <GuestbookMessageCard key={item.id} item={item} divided={index > 0} onReply={onReply} />
      ))}
    </section>
  );
}

function GuestbookMessageCard({ item, divided, onReply }: { item: GuestbookMessage; divided?: boolean; onReply?: (item: GuestbookMessage) => void }) {
  const metaParts = publicMetaParts(item);

  return (
    <article className={`flex gap-4 px-5 py-5 sm:px-6 ${divided ? "border-t border-slate-200/40" : ""}`}>
        ...
        {item.children && item.children.length > 0 && (
          <div className="mt-4 space-y-3 sm:ml-10">
            {item.children.filter((child) => !child.deleted).map((child) => (
              <GuestbookReplyCard key={child.id} item={child} fallbackReplyTo={String(child.parentId) === String(item.id) ? item.nickname : ""} onReply={onReply} />
            ))}
          </div>
        )}
      </div>
...
function GuestbookReplyCard({ item, fallbackReplyTo, onReply }: { item: GuestbookMessage; fallbackReplyTo: string; onReply?: (item: GuestbookMessage) => void }) {
